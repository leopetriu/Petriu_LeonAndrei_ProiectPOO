...
class Locatie 
{
    ...
    Locatie(int zilesoare, int zileploaie)
    {
        this->zilesoare = zilesoare;
        this->zileploaie = zileploaie;
    }

    Locatie(const Locatie& l)
    {
        this->zileploaie = l.zileploaie;
        this->zilesoare = l.zilesoare;
    }
...
//CLASA COPAC
class Copac : public Locatie, public Plante
{
protected:
const int TipPlanta;
...
friend ofstream& operator<<(ofstream& file, Copac copac)
{
file << "Culoare: " << copac.culoare <<endl<< "Inaltime: " << copac.inaltime <<endl<< "Latime: " <<copac.latimeTrunchi <<endl;
for(int i = 0; i<5;i++)
{
    file <<"Crestere Luna "<<i<<": "<<copac.CrestereLuna[i] <<endl;
}
return file;
};

friend ifstream& operator>>(ifstream& file, Copac& copac)
{
char buffer[10];
file >> buffer;
copac.culoare = new char[strlen(buffer) + 1];
copac.culoare.assign(buffer);
file >> copac.inaltime;
file >> copac.latimeTrunchi;
for(int i = 0;i<5;i++)
{
    file >> copac.CrestereLuna[i];
}
...
class Trandafir : public Locatie, public Plante
{
private:
    const int TipPlanta;
    static string ecosistem;
   ...
   Trandafir() : TipPlanta(2)
    {
       ...
       for(int i = 0; i<5;i++)
       {
        this->CrestereLunaTrandafir[i] = 0;
       }
       this->inaltime = 0;
       this->nrPet = 0;
       this->Spini = false;
...
friend ofstream& operator<<(ofstream& file, Trandafir tdf)
{
file << "Culoare: "<<tdf.culoare<<endl;
file << "Inaltime: "<<tdf.inaltime<<endl;
file << "Nr Petale: "<<tdf.nrPet<<endl;
file<< "Spini: "<<tdf.Spini<<endl;
for(int i = 0; i<5;i++)
{
    file<<"Crestere Luna "<<i+1<<": "<<tdf.CrestereLunaTrandafir[i]<<endl;
}
return file;
};

friend ifstream& operator>>(ifstream& file, Trandafir& tdf)
{
char buffer[10];
file >> buffer;
tdf.culoare = new char[strlen(buffer)+1];
tdf.culoare.assign(buffer);
file >> tdf.inaltime;
file >> tdf.nrPet;
file >> tdf.Spini;
for(int i = 0; i<5;i++)
{
    file >> tdf.CrestereLunaTrandafir[i];
}
return file;
}
};
...
class Lavanda : public Locatie, public Plante
{
    private:
    const int TipPlanta;
    static string ecosistem;
...
void scrieInFisierBinarLavanda(fstream& file)
{
int lungime = this->culoare.size();
file.write((char*)&lungime, sizeof(int));
for(int i = 0; i <lungime;i++)
{
    file.write((char*)&this->culoare[i], sizeof(char));
}
file.write((char*)&this->inaltime, sizeof(float));
file.write((char*)&this->aroma, sizeof(bool));
for(int i = 0; i<5;i++)
{
    file.write((char*)&this->CrestereLunaLavanda[i], sizeof(float));
}
file.close();
}

void citesteDinFisierBinarLavanda(fstream& f)
{
    int lungime;
    f.read((char*)&lungime, sizeof(int));
    char* temp = new char[lungime];
    f.read((char*)temp, lungime);
    culoare = temp;
    f.read((char*)&this->inaltime, sizeof(float));
    f.read((char*)&this->aroma, sizeof(bool));
    for(int i = 0; i<5; i++)
    {
        f.read((char*)&this->CrestereLunaLavanda[i], sizeof(float));
    }
f.close();
delete[]temp;
}
...
void scrieFisarBinar(fstream& f)
{
    f.write((char*)&this->trandafir, sizeof(Trandafir));
    f.write((char*)&this->inaltime, sizeof(int));
    f.write((char*)&this->Observare, sizeof(int));
    f.write((char*)&this->plouat, sizeof(bool));
    f.write((char*)&this->cantitateApa, sizeof(float));
    f.close();
}

void citesteFisierBinar(fstream& f)
{   
    f.read((char*)&this->trandafir, sizeof(Trandafir));
    f.read((char*)&this->inaltime, sizeof(int));
    f.read((char*)&this->Observare, sizeof(int));
    f.read((char*)&this->plouat, sizeof(bool));
    f.read((char*)&this->cantitateApa, sizeof(float));
    f.close();
}
...
class CopacCuMere: public Copac
{
private:
    int nrMere;
    string culoareMar;
   ...
   void zona()
{
    ...
    void setCuloareMar(string culoareMar)
    {
        this->culoareMar = culoareMar;
    }

    string getCuloareMar()
    {
        return culoareMar;
    }

    void setnrMere(int nrMere)
    {
        this->nrMere = nrMere;
    }
    ...
    CopacCuMere(int nrMere, string culoareMar, float* CrestereLuna) : Copac(5, 30, 15, CrestereLuna, "verde")
    {
    this->nrMere = nrMere;
    this->culoareMar = culoareMar;
    }

    CopacCuMere(int nrMere, string culoareMar, int TipPlanta, int inaltime, float latimeTrunchi, float *CrestereLuna, string culoare): Copac(TipPlanta, inaltime, latimeTrunchi, CrestereLuna, culoare)
    {
        this->nrMere = nrMere;
        this->culoareMar = culoareMar;
    }

    CopacCuMere(const CopacCuMere& copacMere):Copac(copacMere)
    {
        this->nrMere = copacMere.nrMere;
        this->culoareMar = copacMere.culoareMar;
    }

    CopacCuMere(const Copac& copac):Copac(copac)
    {
        this->nrMere = 4;
        this->culoareMar = "Verde";
    ...
    friend ostream& operator<<(ostream& out, const CopacCuMere copacMere)
    {
        out << "Copac Cu Mere:" <<endl<<"Nr Mere: "<<copacMere.nrMere<<endl<<"Culoarea Marului: "<<copacMere.culoareMar<<endl<<"Inaltime: "<<copacMere.inaltime<<endl<<"Latime: "<<copacMere.latimeTrunchi<<endl<<"Culoare: "<<copacMere.culoare<<endl;
        for(int i = 0; i<5; i++)
        {
            out<<"Crestere in Luna "<<i+1<<": "<<copacMere.CrestereLuna[i]<<endl;
        }
...
Nuc(int nrFructe, string soi, float* CrestereLuna):Copac(6, 35, 23, CrestereLuna, "Verde")
{
    this->nrFructe = nrFructe;
    this->soi = soi;
}

Nuc(int nrFructe, string soi, int TipPlanta, int inaltime, float latimeTrunchi, float* CrestereLuna, string culoare):Copac(TipPlanta, inaltime, latimeTrunchi, CrestereLuna, culoare)
{
    this->nrFructe = nrFructe;
    this->soi = soi;
}

Nuc(const Nuc& CopacNuc):Copac(CopacNuc)
{
    this->nrFructe = CopacNuc.nrFructe;
    this->soi = CopacNuc.soi;
}
~Nuc()
{

}
friend ostream& operator<<(ostream& out, const Nuc CopacNuc)
    {
        out << "Copac (Nuc):" <<endl<<"Nr Fructe: "<<CopacNuc.nrFructe<<endl<<"Culoarea Marului: "<<CopacNuc.soi<<endl<<"Inaltime: "<<CopacNuc.inaltime<<endl<<"Latime: "<<CopacNuc.latimeTrunchi<<endl<<"Culoare: "<<CopacNuc.culoare<<endl;
        for(int i = 0; i<5; i++)
        {
            out<<"Crestere in Luna "<<i+1<<": "<<CopacNuc.CrestereLuna[i]<<endl;
        }
...
Copac copac5(1, 8, 6, CrestereLunaCopac, "Verzi");
ofstream fis ("fisiertextcopac.txt", ios::out);
fis << copac5;

Copac copacDinFisierTxt;
ifstream fisIn("copactxt.txt", ios::in);
fisIn >> copacDinFisierTxt;
cout << copacDinFisierTxt;
...
Trandafir tdf7(2, 4, 20, CrestereLunaTrandafir, true, "Rosie");
ofstream fistdf ("fisiertrandafir.txt", ios::out);
fistdf << tdf7;

Trandafir trandafirDinFisierTxt;
ifstream fisIntdf("fisiertrandafir2.txt", ios::in);
fisIntdf >> trandafirDinFisierTxt;
cout<<trandafirDinFisierTxt;
...
Lavanda lv6(3, 5, "violet", CrestereLunaLavanda, true);
cout<<lv6;
fstream fisier("Lavanda.txt", ios::out|ios::binary);
lv6.scrieInFisierBinarLavanda(fisier);

fstream fisiercitit("Lavanda.txt", ios::in|ios::binary);
Lavanda lvdDinFisier;
lvdDinFisier.citesteDinFisierBinarLavanda(fisiercitit);
cout<<lvdDinFisier;
...
ip3.afisare();
cout<<endl;
ingrizirePlante ip4(&tdf6, 20, false, tdf6.getInaltime());
fstream fisierip("FisierBinarIP.txt", ios::out|ios::binary);
ip4.scrieFisarBinar(fisierip);
ingrizirePlante ipDinFisier;
fstream fisieripCitit("FisierBinarIP.txt", ios::in|ios::binary);
ipDinFisier.citesteFisierBinar(fisieripCitit);
ipDinFisier.afisare();
*/

CopacCuMere copacMere1;
CopacCuMere copacMere2(80, "Galben", CrestereLunaCopac);

CopacCuMere copacMere3(70, "Rosu", 5, 30, 8, CrestereLunaCopac, "Verde");

CopacCuMere copacMere4 = copacMere3;

CopacCuMere copacMere5(copac3);

cout << copacMere1 <<endl;
cout << copacMere2 <<endl;

cout << copacMere3<<endl;

cout << copacMere4<<endl; 

cout<<copacMere5<<endl;

copacMere2.setnrMere(100);
cout<<copacMere2.getnrMere()<<endl;
copacMere3.setCuloareMar("Verde");
cout<<copacMere3.getCuloareMar()<<endl;

Nuc CopacNuc1;
Nuc CopacNuc2(70, "Argesean",CrestereLunaCopac);
Nuc CopacNuc3(90, "Jupanesti", 6, 35, 9, CrestereLunaCopac, "Verde");
Nuc CopacNuc4 = CopacNuc3;
cout << CopacNuc1<<endl;
cout << CopacNuc2<<endl;
cout << CopacNuc3<<endl;
cout << CopacNuc4<<endl;

CopacNuc1.setnrFructe(50);
cout<<CopacNuc1.getnrFructe()<<endl;
CopacNuc2.setsoi("Novaci");
cout<<CopacNuc2.getsoi()<<endl;
...
loc[2] = new Lavanda();
loc[3] = new Nuc();
loc[4] = new CopacCuMere();
loc[5] = new Copac();
loc[6] = new Trandafir();
loc[7] = new Lavanda();
loc[8] = new Nuc();
loc[9] = new CopacCuMere();
...
plante[0] = new Copac();
plante[1] = new Trandafir();
plante[2] = new Lavanda();
plante[3] = new Nuc();
plante[4] = new CopacCuMere();
plante[5] = new Copac();
plante[6] = new Trandafir();
plante[7] = new Lavanda();
plante[8] = new Nuc();
plante[9] = new CopacCuMere();
